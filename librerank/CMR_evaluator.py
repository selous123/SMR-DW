...
class CMR_evaluator(EGR_evaluator):
    ...
    def dnn_layer(self):
        dnn_layer = [self.sum_pooling_layer, self.concatenation_layer,
                     self.multi_head_self_attention_layer, self.rnn_layer, self.pair_wise_comparison_layer] #[bs,dim]
        dnn_layer = tf.concat(values=dnn_layer, axis=-1) #[bs,dim]
    ...
    def sum_pooling_channel(self):
        with tf.variable_scope(name_or_scope="{}_Sum_Pooling_Channel".format(self.name)):
            self.sum_pooling_layer = tf.reduce_sum(self.all_feature_concatenation, axis=1) #[bs,ls,dim]->[bs,dim]
    ...
    def logits_layer(self):
        ...
        ## 这里的实现有很大问题; 建议用EGR_evaluator;
        elif self.score_format == 'iv': #坑位级别
            logits = layers.linear(self.final_neurons, self.max_time_len) #[bs, max_time_len]
